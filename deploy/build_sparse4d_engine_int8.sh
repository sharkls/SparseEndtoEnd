...
# --profilingVerbosity=detailed: 开启详细耗时统计
INT8_ARGS="--int8 --fp16 --profilingVerbosity=detailed"
...
# 准备安全输入路径 (对于 Head2，某些输入需要特定的 shape 范围)
...
if [[ -f "${SAFE_INPUT_DIR}/spatial_shapes.bin" && -f "${SAFE_INPUT_DIR}/level_start_index.bin" ]]; then
    echo "[INFO] Using safe inputs for trtexec to avoid potential shape inference issues."
