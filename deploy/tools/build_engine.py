...
class FileCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def write_calibration_cache(self, cache):
        ...
        with open(self.cache_file, "wb") as f:
            f.write(cache)
        # NOTE: We intentionally do NOT free CUDA buffers here (or in __del__)
        # because TensorRT / cuda-python lifetime interactions can easily cause
        # double-free or heap corruption when the calibrator is destroyed.
        # This script is a short-lived build tool, so leaking a small amount of
        # calibration GPU memory is acceptable and avoids "double free or
        # corruption (!prev)" aborts during INT8 calibration.
...
def build_engine(onnx_path, engine_path, head_type, mode="fp16", plugins=[], calib_dir=None, explicit_precision=False):
    ...
    if mode == "int8" and explicit_precision:
        ...
        for i in range(network.num_layers):
            ...
            if is_matmul:
                # Exclude Attention layers if they are sensitive (often Softmax related, but MatMul in attention might also be sensitive)
                # Let's be aggressive first for FFNs
                if "attn" in name or "Attention" in name:
                    # Skip attention matmuls for now to be safe, or maybe include them?
                    # The FFN layers in Sparse4D usually look like /layers.X/layers/layers.X/MatMul
                    # The Attention layers look like /layers.X/attn/MatMul
                    print(f"[INFO] Skipping Attention MatMul: {name}")
                    count_skip += 1
                    continue
                
                # Check for FFN or other dense layers
