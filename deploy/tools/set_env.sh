...
################***EnvVersion-4***#################
###     LinuxPlatform:            aarch64 (Jetson Orin)                                              ###
###     TensorRT :                    8.6.2.3                                                          ###
###     CUDA :                             12.2.140                                                      ###
###     cuDNN:                          8.9.4.25                                                       ###
###     CUDA capability :        sm_87                                                         ###
################***EnvVersion-4***#################

EnvVersion=4        # 设置为4以使用Orin平台 (TensorRT 8.6.2.3 + CUDA 12.2 + cuDNN 8.9.4)
...
else    # EnvVersion = 4 (Jetson Orin)
    export ENV_TensorRT_LIB=/usr/lib/aarch64-linux-gnu
    export ENV_TensorRT_INC=/usr/include/aarch64-linux-gnu
    export ENV_TensorRT_BIN=/usr/src/tensorrt/bin
    export ENV_CUDA_LIB=/usr/local/cuda-12.2/lib64
    export ENV_CUDA_INC=/usr/local/cuda-12.2/include
    export ENV_CUDA_BIN=/usr/local/cuda-12.2/bin
    export ENV_cuDNN_LIB=/usr/lib/aarch64-linux-gnu  # 导入 cuDNN 8.9.4 库路径
    export PATH=/usr/local/cuda-12.2/bin:$PATH
fi


# 检查 TensorRT 可执行文件是否存在 (Orin平台可能在/usr/bin)
if [ ! -f "${ENV_TensorRT_BIN}/trtexec" ] && [ ! -f "/usr/bin/trtexec" ]; then
    echo "[ERROR] Failed to Find trtexec in ${ENV_TensorRT_BIN} or /usr/bin!"
    return
fi
# 如果/usr/bin/trtexec存在但ENV_TensorRT_BIN中不存在，使用/usr/bin
if [ ! -f "${ENV_TensorRT_BIN}/trtexec" ] && [ -f "/usr/bin/trtexec" ]; then
    export ENV_TensorRT_BIN=/usr/bin
fi
...
if [ -f "dfa_plugin/tools/cudasm.sh" ]; then
    ...
    # 根据平台设置默认CUDASM值
    if [ $EnvVersion = 4 ]; then
        echo "[WARNING] Using default CUDASM=87 for Orin platform"
        cudasm=87
    else
...
export PATH=$ENV_TensorRT_BIN:$ENV_CUDA_BIN:$PATH
