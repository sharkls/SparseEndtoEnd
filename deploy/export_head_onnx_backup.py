        ...
        for i, op in enumerate(self.operation_order):
            ...
            if self.layers[i] is None:
                ...
                if len(prediction) == self.num_single_frame_decoder:
                    # 修复：使用与Python推理一致的InstanceBank逻辑
                    N = (
                        self.instance_bank.num_anchor
                        - self.instance_bank.num_temp_instances
                    )
                    
                    # 修复1：正确的置信度计算（先sigmoid）
                    confidence = cls.max(dim=-1).values.sigmoid()
                    
                    # 修复2：置信度融合逻辑
                    if self.instance_bank.confidence is not None:
                        confidence[:, :self.instance_bank.num_temp_instances] = torch.maximum(
                            self.instance_bank.confidence * self.instance_bank.confidence_decay,
                            confidence[:, :self.instance_bank.num_temp_instances],
                        )
                    
                    # 修复3：使用融合后的置信度进行TopK选择
