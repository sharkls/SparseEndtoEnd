...
# -*- coding: utf-8 -*-
"""
Backbone Engine Validation Script
==================================
验证 Backbone TensorRT Engine 与 PyTorch 中间结果的一致性

Usage:
    # 验证 Backbone FP32
    /bin/python3 validate_backbone_engine.py --precision fp32 --data_dir val_data_e2e_fp32
    
    # 验证 Backbone FP16
    /bin/python3 validate_backbone_engine.py --precision fp16 --data_dir val_data_e2e_fp32
    
    # 验证所有精度
    /bin/python3 validate_backbone_engine.py --precision all --data_dir val_data_e2e_fp32
    
    # 验证多个 sample
    /bin/python3 validate_backbone_engine.py --precision fp32 --samples 0 1 2
"""
...
# Backbone 验证
# ============================================================================
def validate_backbone(engine_path, data_dir, sample_idx=0, verbose=False):
    """验证 Backbone Engine"""
    print(f"\n{'#'*80}")
    print(f"# Validating Backbone Engine")
    ...
    # Backbone 输入映射
    # Backbone 输入是图像: imgs (1, 6, 3, 256, 704)
    # Backbone 输出是特征: feature (1, 89760, 256), spatial_shapes, level_start_index
    backbone_input_mapping = {
        "img": "imgs",  # engine input name -> data file name
    }
    ...
    for engine_name in wrapper.input_names:
        # 查找对应的 bin 文件
        data_name = backbone_input_mapping.get(engine_name, engine_name)
        path = find_file(data_dir, data_name, sample_prefix)
        
        if not path:
            # 尝试其他可能的名称
            alt_names = ["imgs", "img", "image", "images"]
            for alt in alt_names:
                path = find_file(data_dir, alt, sample_prefix)
                if path:
                    break
    ...
    inference_time = (time.time() - start_time) * 1000 / num_runs
    print(f"Inference time (avg of {num_runs} runs): {inference_time:.2f} ms")
    
    # Backbone 输出映射
    backbone_output_mapping = {
        "feature": "feature",
    ...
    for name in wrapper.output_names:
        ...
        # 查找对应的 PyTorch 输出
        data_name = backbone_output_mapping.get(name, name)
        pt_path = find_file(data_dir, data_name, sample_prefix)
        if not pt_path:
        ...
        try:
            ...
            # 尝试其他方式比较
            if pt_out.size == engine_out.size:
                pt_out = pt_out.reshape(engine_out.shape)
            else:
                print(f"         Size mismatch: engine={engine_out.size}, pt={pt_out.size}")
                continue
    ...
    print_error_table(metrics_list, "Backbone Output Comparison")
    ...
    if metrics_list:
        ...
        for m in metrics_list:
            if "max_rel_diff" in m:
                print(f"  {m['name']}:")
                print(f"    - Max Relative Diff: {m.get('max_rel_diff', 'N/A'):.6e}")
                print(f"    - Mean Relative Diff: {m.get('mean_rel_diff', 'N/A'):.6e}")
                print(f"    - Std Diff: {m.get('std_diff', 'N/A'):.6e}")
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="Validate Backbone TensorRT Engine")
    ...
    parser.add_argument("--samples", type=int, nargs='+', default=[0],
                        help="Sample indices to validate (default: 0)")
    ...
    print("="*80)
    print(" Backbone Engine Validation Tool")
    print("="*80)
    ...
    for precision in precisions:
        # 构造 engine 路径
        if precision == "fp32":
            engine_name = "sparse4dbackbone.engine"
        else:  # fp16
            engine_name = "sparse4dbackbone-fp16.engine"
        ...
        if not os.path.exists(engine_path):
            # 尝试备用名称
            alt_names = [
                f"sparse4dbackbone-{precision}.engine",
                f"sparse4dbackbone_{precision}.engine",
                "sparse4dbackbone.engine" if precision == "fp16" else "sparse4dbackbone-fp16.engine"
            ]
            for alt in alt_names:
                alt_path = os.path.join(args.engine_dir, alt)
        ...
        for sample_idx in args.samples:
            key = f"backbone_{precision}_sample{sample_idx}"
            
            results[key] = validate_backbone(
