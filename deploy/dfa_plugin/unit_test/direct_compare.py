#!/usr/bin/env python3
"""
直接对比 PyTorch DFA CUDA 实现和 TRT 插件实现
"""
import os
import sys
import numpy as np
...
def build_and_run_trt_dfa(value, spatial_shapes, level_start_index, sampling_locations, attention_weights):
    ...
    # 导出 ONNX
    onnx_file = "/tmp/tmp_dfa_direct.onnx"
    ...
    output = torch.empty(tuple(output_shape), dtype=torch.float32, device='cuda')
...
def main():
    ...
    # 加载输入
    value = load_bin(f"{data_dir}/sample_0_value_1*89760*256_float32.bin", (1, 89760, 256), np.float32)
    ...
    sampling_locations = load_bin(f"{data_dir}/sample_0_sampling_locations_1*900*13*6*2_float32.bin", (1, 900, 13, 6, 2), np.float32)
    attention_weights = load_bin(f"{data_dir}/sample_0_attention_weights_1*900*13*6*4*8_float32.bin", (1, 900, 13, 6, 4, 8), np.float32)
    ...
    print("\n" + "=" * 60)
    print("Running PyTorch CUDA implementation...")
    ...
    pytorch_output = e2e_deformable_aggregation_ext.deformable_aggregation_forward(
        ...
        attention_weights.contiguous().float()
    )
    ...
    print("\n" + "=" * 60)
    print("Running TRT DFA Plugin...")
    
    trt_output = build_and_run_trt_dfa(value, spatial_shapes, level_start_index, sampling_locations, attention_weights)
