...
def main():
    ...
    # Force loading GT score if track_id is present to avoid mismatch
    has_track_id_input = "track_id" in head2.input_names
    
    if path_cls_gt:
        print("  [InstanceBank] Using GT Class Scores for TopK Selection (Alignment)")
        gt_cls = load_bin(path_cls_gt, shape=(1, 900, 10))
        gt_cls_t = torch.from_numpy(gt_cls).cuda()
        confidence = gt_cls_t.sigmoid().max(dim=-1).values
    else:
        if has_track_id_input:
             print("\n  [CRITICAL WARN] 'pred_class_score' file NOT found but 'track_id' is required!")
             print("  Using Engine Scores for TopK will likely cause Mismatch with file-loaded 'track_id'.")
             print("  Resulting visualization will be wrong (Input Mismatch).\n")
        
        print("  [InstanceBank] Using Predicted Class Scores for TopK Selection (May cause misalignment)")
        pred_cls = head1_out["pred_class_score"] 
        confidence = pred_cls.sigmoid().max(dim=-1).values
    ...
    # =====================================================================
    # PURE INFERENCE SIMULATION (Head1 -> InstanceBank Logic -> Head2)
    # =====================================================================
    print("\n>>> Running Pure Inference Simulation (Head1 -> Head2)")
    ...
    if l2g_0 is not None and g2l_1 is not None:
        print(f"  [Sim] Projecting anchors with dt={dt:.4f}s")
    ...
    print("  [Sim] Running Head2 Inference...")
    ...
    if args.visualize:
        ...
        print(f"  [Sim] Engine Detection: {len(sim_res_f1['boxes'])} boxes")
