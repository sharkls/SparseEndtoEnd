...
def obtain_sensor2top(
    ...
    R = (l2e_r_s_mat.T @ e2g_r_s_mat.T) @ (                         # camera2global rotation matrix
        np.linalg.inv(e2g_r_mat).T @ np.linalg.inv(l2e_r_mat).T     # global2lidar rotation matrix 
    )
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in tqdm(nusc.sample, desc="Processing Sample"):
        ...
        info = {
            ...
            "ego2global_translation": pose_record["translation"],   # listlen=3
            "ego2global_rotation": pose_record["rotation"],  # listlen=4
        ...
        l2e_r_mat = Quaternion(l2e_r).rotation_matrix  # numpy(3,3)     lidar2ego rotation matrix
        e2g_r_mat = Quaternion(e2g_r).rotation_matrix  # numpy(3,3)     ego2global rotation matrix
        ...
        if not test:
            ...
            # convert velo from global to lidar 将速度转换到激光雷达坐标系
            for i in range(len(boxes)):
                velo = np.array([*velocity[i], 0.0])
                velo = velo @ np.linalg.inv(e2g_r_mat).T @ np.linalg.inv(l2e_r_mat).T
                velocity[i] = velo[:2]

            names = [b.name for b in boxes]  # 类型转换
