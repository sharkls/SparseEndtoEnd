...
/**
 * @brief GPU NMS函数 - 直接处理原始输入数据（float版本）
...
/**
 * @brief GPU NMS函数 - 直接处理原始输入数据（half版本）
 * @param pred_anchor GPU上的锚点数据（half精度）
 * @param pred_class_score GPU上的类别分数数据（half精度）
 * @param pred_quality_score GPU上的质量分数数据（half精度）
 * @param pred_track_ids GPU上的跟踪ID数据
 * @param num_objects 目标数量
 * @param num_classes 类别数量
 * @param d_output_boxes GPU上的输出边界框数据
