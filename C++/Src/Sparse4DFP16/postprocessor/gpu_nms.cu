...
// 第一步：转换和过滤内核（half版本，只进行置信度过滤）
__global__ void convert_and_filter_kernel_half(const half* pred_anchor,
                                              const half* pred_class_score,
                                              const half* pred_quality_score,
                                              const int32_t* pred_track_ids,
                                              BoundingBox3D* temp_boxes,
                                              int* valid_count,
                                              int num_objects,
    ...
    // 计算置信度
    // 1. 对class_score应用sigmoid并找到最大值（转换为float进行计算）
    int class_offset = idx * num_classes;
    float max_score = -1e10f;  // 初始化为很小的值
    int best_class = 0;
    
    for (int c = 0; c < num_classes; ++c) {
        float class_score_float = __half2float(pred_class_score[class_offset + c]);
        float sigmoid_score = sigmoid_f(class_score_float);
        if (sigmoid_score > max_score) {
            max_score = sigmoid_score;
            best_class = c;
        }
    ...
    // quality_score的实际格式：每个对象有2个值，索引为 idx * 2 + 0 和 idx * 2 + 1
    int quality_offset = idx * 2;  // quality_dims = 2
    float centerness_raw = __half2float(pred_quality_score[quality_offset + 0]);  // 取第一个维度（centerness）
    float centerness = sigmoid_f(centerness_raw);  // 应用sigmoid
    ...
    // 位置信息（转换为float）
    box.x = __half2float(pred_anchor[anchor_offset + 0]);
    box.y = __half2float(pred_anchor[anchor_offset + 1]);
    box.z = __half2float(pred_anchor[anchor_offset + 2]);
    
    // 尺寸信息：模型输出的是log值，需要应用exp变换
    box.w = expf(__half2float(pred_anchor[anchor_offset + 3]));  // exp(log(w)) = w
    box.l = expf(__half2float(pred_anchor[anchor_offset + 4]));  // exp(log(l)) = l
    box.h = expf(__half2float(pred_anchor[anchor_offset + 5]));  // exp(log(h)) = h
    
    // 航向角：从anchor[6]读取sin(yaw)，从anchor[7]读取cos(yaw)
    const float sin_yaw = __half2float(pred_anchor[anchor_offset + 6]);
    const float cos_yaw = __half2float(pred_anchor[anchor_offset + 7]);
    box.yaw = atan2f(sin_yaw, cos_yaw);
...
// GPU NMS直接处理原始输入数据（half版本）
int gpu_nms_direct(const half* pred_anchor,
                   const half* pred_class_score,
                   const half* pred_quality_score,
                   const int32_t* pred_track_ids,
                   int num_objects,
    ...
    try {
        ...
        // 第一步：转换和过滤（half版本，只进行置信度过滤）
        convert_and_filter_kernel_half<<<grid_size, block_size, 0, stream>>>(
            pred_anchor,
            pred_class_score,
            pred_quality_score,
            pred_track_ids,
        ...
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            LOG(ERROR) << "[ERROR] CUDA error in convert kernel (half): " << cudaGetErrorString(err);
            cudaFree(d_all_boxes);
            cudaFree(d_temp_boxes);
            cudaFree(d_valid_count);
            cudaFree(d_suppressed);
        ...
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            LOG(ERROR) << "[ERROR] CUDA error in NMS kernel (half): " << cudaGetErrorString(err);
            cudaFree(d_all_boxes);
            cudaFree(d_temp_boxes);
            cudaFree(d_valid_count);
            cudaFree(d_suppressed);
        ...
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            LOG(ERROR) << "[ERROR] CUDA error in compact kernel (half): " << cudaGetErrorString(err);
            cudaFree(d_all_boxes);
            cudaFree(d_temp_boxes);
            cudaFree(d_valid_count);
            cudaFree(d_suppressed);
        ...
        LOG(ERROR) << "[ERROR] Exception in GPU NMS direct (half): " << e.what();
        return -1;
    }
}
