...
std::vector<BoundingBox3D> PostprocessorImplement::to_bounding_box_3d(const common::HeadOutput& head_output)
{
    ...
    // 将GPU数据拷贝到CPU缓存（half类型需要转换为float）- 只转换一次，避免在循环中重复转换
    const std::vector<half> host_anchor_half = head_output.pred_anchor.cudaMemcpyD2HResWrap();
    const std::vector<half> host_cls_half = head_output.pred_class_score.cudaMemcpyD2HResWrap();
    const std::vector<half> host_q_half = head_output.pred_quality_score.cudaMemcpyD2HResWrap();
    const std::vector<int32_t> host_track = head_output.pred_track_ids.cudaMemcpyD2HResWrap();
    
    // 转换为float用于CPU计算（只转换一次）
    std::vector<float> host_anchor(host_anchor_half.size());
    std::vector<float> host_cls(host_cls_half.size());
    std::vector<float> host_q(host_q_half.size());
    for (size_t j = 0; j < host_anchor_half.size(); ++j) {
        host_anchor[j] = __half2float(host_anchor_half[j]);
    }
    for (size_t j = 0; j < host_cls_half.size(); ++j) {
        host_cls[j] = __half2float(host_cls_half[j]);
    }
    for (size_t j = 0; j < host_q_half.size(); ++j) {
        host_q[j] = __half2float(host_q_half[j]);
    }
