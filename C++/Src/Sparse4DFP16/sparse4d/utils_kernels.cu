...
// ==================== Half 精度版本的 Kernel 实现 ====================

// 置信度衰减（half版本）
__global__ void applyConfidenceDecayKernelHalf(const half* input_confidence,
    ...
    if (tid < array_size) {
        float input_val = __half2float(input_confidence[tid]);
        float output_val = __fmul_rn(input_val, decay_factor);
        output_confidence[tid] = __float2half(output_val);
    }
}

// 置信度融合（half版本）
__global__ void fuseConfidenceKernelHalf(half* output_confidence,
    ...
    if (tid < size) {
        float current_val = __half2float(current_confidence[tid]);
        float cached_val = __half2float(cached_confidence[tid]);
        output_confidence[tid] = __float2half(fmaxf(current_val, cached_val));
    }
}

// 锚点投影（half版本）
__global__ void anchorProjectionKernelHalf(half* temp_anchor,
                                           Mat4 transform,
                                           float time_interval,
                                           uint32_t topk_querys,
                                           uint32_t query_dims) {
    ...
    // 更新锚点数据（转换回half）
    anchor_ptr[0] = __float2half(new_center_x);
    anchor_ptr[1] = __float2half(new_center_y);
    anchor_ptr[2] = __float2half(new_center_z);
    anchor_ptr[3] = __float2half(size_x);
    anchor_ptr[4] = __float2half(size_y);
    anchor_ptr[5] = __float2half(size_z);
    anchor_ptr[6] = __float2half(new_yaw_x);
    anchor_ptr[7] = __float2half(new_yaw_y);
    anchor_ptr[8] = __float2half(new_vel_x);
    anchor_ptr[9] = __float2half(new_vel_y);
    anchor_ptr[10] = __float2half(new_vel_z);
}

// 最大置信度（half版本）
__global__ void getMaxConfidenceScoresKernelHalf(const half* confidence_logits,
    ...
    max_confidence_scores[idx] = __float2half(sigmoid_value);
}

// 按索引复制TopK数据（half版本）
__global__ void copyTopKDataFromIndicesKernelHalf(const half* instance_feature,
                                                  const half* anchor,
    ...
    uint32_t src_idx = topk_indices[idx];
    
    // 复制置信度
    output_confidence[idx] = confidence[src_idx];
    
    // 复制实例特征
    for (uint32_t i = 0; i < embedfeat_dims; ++i) {
        output_instance_feature[idx * embedfeat_dims + i] = 
            instance_feature[src_idx * embedfeat_dims + i];
    }
    
    // 复制锚点
    for (uint32_t i = 0; i < query_dims; ++i) {
        output_anchor[idx * query_dims + i] = anchor[src_idx * query_dims + i];
    }
    
    // 复制track_id（使用原始索引）
    output_track_ids[idx] = src_idx;
}

// ==================== Half 精度版本的 C++ 接口实现 ====================

// Half到Float转换kernel（用于排序）
__global__ void convertHalfToFloatKernel(const half* input, float* output, uint32_t size) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
...
Status getTopkInstanceOnGPUThrust(const CudaWrapper<half>& confidence,
                                 ...
                                 CudaWrapper<int32_t>& output_track_ids,
                                 cudaStream_t stream) {
    // 创建索引-值对（使用float进行排序，因为thrust对half支持有限）
    thrust::device_vector<float> d_values(num_querys);
    thrust::device_vector<uint32_t> d_indices(num_querys);
    ...
    // 在GPU上直接将half转换为float（避免D2H/H2D转换）
    uint32_t block_size = 256;
    uint32_t grid_size = (num_querys + block_size - 1) / block_size;
    convertHalfToFloatKernel<<<grid_size, block_size, 0, stream>>>(
        confidence.getCudaPtr(), d_values.data().get(), num_querys
    );
    ...
    // 提取前k个结果
    grid_size = (num_topk_querys + block_size - 1) / block_size;
    
    copyTopKDataFromIndicesKernelHalf<<<grid_size, block_size, 0, stream>>>(
        instance_feature.getCudaPtr(),
        anchor.getCudaPtr(),
        confidence.getCudaPtr(),
        d_indices.data().get(),
...
Status applyConfidenceDecayOnGPU(const CudaWrapper<half>& input_confidence,
    ...
    applyConfidenceDecayKernelHalf<<<grid_size, block_size, 0, stream>>>(
        input_confidence.getCudaPtr(), output_confidence.getCudaPtr(), decay_factor, array_size
    );
...
Status fuseConfidenceOnGPU(const CudaWrapper<half>& current_confidence,
    ...
    fuseConfidenceKernelHalf<<<grid_size, block_size, 0, stream>>>(
        output_confidence.getCudaPtr(),
...
Status anchorProjectionOnGPU(CudaWrapper<half>& temp_anchor,
    ...
    anchorProjectionKernelHalf<<<grid_size, block_size, 0, stream>>>(
        temp_anchor.getCudaPtr(), hmat, time_interval, topk_querys, query_dims
    );
...
Status getMaxConfidenceScoresOnGPU(const CudaWrapper<half>& confidence_logits,
    ...
    getMaxConfidenceScoresKernelHalf<<<grid_size, block_size, 0, stream>>>(
