...
private:
    ...
    void updateConfidence(const CudaWrapper<half>& confidence);
    ...
    /// @param stream CUDA流
    void anchorProjection(CudaWrapper<half>& temp_anchor,
    ...
    // GPU内存包装器
    CudaWrapper<half> m_gpu_instance_feature_;     // 实例特征（900， 256）
    CudaWrapper<half> m_gpu_anchor_;               // 锚点(900, 11)
    CudaWrapper<half> m_gpu_confidence_;           // 置信度(900)
    CudaWrapper<int32_t> m_gpu_track_ids_;          // 跟踪ID(900)
    CudaWrapper<half> m_gpu_cached_feature_;       // 缓存的特征(600, 256)
    CudaWrapper<half> m_gpu_cached_anchor_;        // 缓存的锚点(600, 11)
    CudaWrapper<half> m_gpu_cached_confidence_;    // 缓存的置信度(600)
...
{   
    ...
    cudaError_t err = cudaMemcpyAsync(
        ...
        instance_feature_copy_size * sizeof(half),
    ...
    if (!is_first_frame) {
        ...
        err = cudaMemcpyAsync(
            ...
            temp_feature_copy_size * sizeof(half),
            cudaMemcpyDeviceToDevice,
            stream
        );
        if (err != cudaSuccess) {
        ...
        err = cudaMemcpyAsync(
            ...
            temp_anchor_copy_size * sizeof(half),
            cudaMemcpyDeviceToDevice,
            stream
        );
        if (err != cudaSuccess) {
    ...
    std::vector<half> time_interval_vec = {__float2half(time_interval_)};
...
Status InstanceBankImplement::cache(common::HeadOutput& head_output, const bool& is_first_frame, cudaStream_t stream) 
{
    ...
    // 1. 获取最大置信度分数
    CudaWrapper<half> confidence = InstanceBankImplement::getMaxConfidenceScores(head_output.pred_class_score, num_anchors_);
...
void InstanceBankImplement::anchorProjection(CudaWrapper<half>& temp_anchor,
                                       const Eigen::Matrix<float, 4, 4>& temp_to_cur_mat,
                                       const float& time_interval,
                                       cudaStream_t stream) {
...
bool InstanceBankImplement::loadDataFromFile(const std::string& file_path, size_t expected_size, CudaWrapper<T>& data) 
{
    ...
    if constexpr (std::is_same_v<T, half>) {
        ...
        file.read(reinterpret_cast<char*>(temp_float_buffer.data()), expected_file_size);
        file.close();
        
        // 转换为half
        std::vector<half> temp_buffer(expected_size);
        ...
        // 其他类型：直接读取
        size_t expected_file_size = expected_size * sizeof(T);
        if (file_size != expected_file_size) {
            LOG(ERROR) << "[ERROR] File size mismatch. Expected: " << expected_file_size 
