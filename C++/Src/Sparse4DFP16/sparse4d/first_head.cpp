...
{   
    ...
    // 检查输入缓冲区是否为空
    if (pipeline_context.features.getCudaPtr() == nullptr ||
        pipeline_context.spatial_shapes.getCudaPtr() == nullptr ||
        pipeline_context.level_start_index.getCudaPtr() == nullptr ||
        pipeline_context.instance_feature.getCudaPtr() == nullptr ||
        pipeline_context.anchor.getCudaPtr() == nullptr ||
        pipeline_context.time_interval.getCudaPtr() == nullptr ||
        pipeline_context.image_wh.getCudaPtr() == nullptr ||
        pipeline_context.lidar2img.getCudaPtr() == nullptr) {
        LOG(ERROR) << "[ERROR] Input buffer is null in first head forward";
        return Status::kInferenceErr;
    }
    
    // 检查输出缓冲区是否为空
    if (head_output.pred_instance_feature.getCudaPtr() == nullptr ||
        head_output.pred_anchor.getCudaPtr() == nullptr ||
        head_output.pred_class_score.getCudaPtr() == nullptr ||
        head_output.pred_quality_score.getCudaPtr() == nullptr) {
        LOG(ERROR) << "[ERROR] Output buffer is null in first head forward";
        return Status::kInferenceErr;
    }
    
    // 准备输入输出缓冲区
    std::vector<void*> input_buffers = {
        const_cast<void*>(static_cast<const void*>(pipeline_context.features.getCudaPtr())),
    ...
    std::vector<void*> output_buffers = {
        //  static_cast<void*>(head_output.tmp_outs0.getCudaPtr()),
    ...
    for (size_t i = 0; i < input_buffers.size(); ++i) {
        if (input_buffers[i] == nullptr) {
            LOG(ERROR) << "[ERROR] Input buffer " << i << " is null";
            return Status::kInferenceErr;
        }
    }
    for (size_t i = 0; i < output_buffers.size(); ++i) {
        if (output_buffers[i] == nullptr) {
            LOG(ERROR) << "[ERROR] Output buffer " << i << " is null";
            return Status::kInferenceErr;
        }
    }
    ...
    if (pre_err != cudaSuccess) {
        LOG(ERROR) << "[ERROR] CUDA error before inference: " << cudaGetErrorString(pre_err);
        cudaDeviceSynchronize();
    }
    
    // 执行推理
    // LOG(INFO) << "[DEBUG] First head inference starting...";
    ...
    if (post_err != cudaSuccess) {
        LOG(ERROR) << "[ERROR] CUDA error after inference: " << cudaGetErrorString(post_err);
        return Status::kInferenceErr;
    }
    
    if (success) {
        // LOG(INFO) << "[DEBUG] First head inference completed successfully";
        return Status::kSuccess;
    } else {
        LOG(ERROR) << "[ERROR] First head inference failed";
        return Status::kInferenceErr;
    }
}
