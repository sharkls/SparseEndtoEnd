...
bool SparseBEV::init(void* p_pAlgParam) 
{
    ...
    // LOG(INFO) << "[INFO] Task config loaded, datatype: " << static_cast<int>(datatype_);
    ...
    // // 
    // if(datatype_ == sparsebev::DataType::float)
    // {
    //     bool is_success = m_float_input_wrapper.allocate(input_size);
    // }
    // else if(datatype_ == sparsebev::DataType::half)
    // {
    //     bool is_success = m_half_input_wrapper.allocate(input_size);
    // }
...
bool SparseBEV::setImageFeature(const sparsebev::ImageFeature& image_data)
{
    ...
    // 依次尝试三种可能的精度
    if (auto p = std::get_if<std::shared_ptr<CudaWrapper<float>>>(&image_data.feature); p && *p) {
        auto float_data = (*p)->cudaMemcpyD2HResWrap();
    m_float_input_wrapper = CudaWrapper<float>(float_data);
        LOG(INFO) << "[INFO] Image feature set successfully (float)";
        return true;
    }
    if (auto p = std::get_if<std::shared_ptr<CudaWrapper<half>>>(&image_data.feature); p && *p) {
        auto half_data = (*p)->cudaMemcpyD2HResWrap();
        // 若需要 float 输入，这里可转换；当前先直接持有 half 到专用 wrapper（如存在）
        // 先转为 float 输入缓存
        std::vector<float> as_float(half_data.size());
        for (size_t i = 0; i < half_data.size(); ++i) as_float[i] = __half2float(half_data[i]);
        m_float_input_wrapper = CudaWrapper<float>(as_float);
        LOG(INFO) << "[INFO] Image feature set successfully (half->float copy)";
        return true;
    }
    if (auto p = std::get_if<std::shared_ptr<CudaWrapper<int8_t>>>(&image_data.feature); p && *p) {
        auto int8_data = (*p)->cudaMemcpyD2HResWrap();
        // 若需要转 float，这里做简单归一或直接静态转换为 float 占位
        std::vector<float> as_float(int8_data.size());
        for (size_t i = 0; i < int8_data.size(); ++i) as_float[i] = static_cast<float>(int8_data[i]);
        m_float_input_wrapper = CudaWrapper<float>(as_float);
        LOG(INFO) << "[INFO] Image feature set successfully (int8->float copy)";
    return true;
    }

    LOG(ERROR) << "[ERROR] Unsupported image feature variant type";
    return false;
}
