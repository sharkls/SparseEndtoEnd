...
// 注册模块
REGISTER_MODULE("SparseBEV", SparseBEVFP16, SparseBEVFP16)

/**
 * @brief 析构函数
 */
SparseBEVFP16::~SparseBEVFP16() {
    // 清理资源
}
 ...
 * @return 初始化是否成功
 */
bool SparseBEVFP16::init(void* p_pAlgParam) 
{
    LOG(INFO) << "[INFO] SparseBEVFP16::init start";
    ...
    if (!m_half_input_wrapper.allocate(input_size) ||
        ...
        !m_half_tmp_outs0_wrapper.allocate(pred_size * 2) ||
        !m_half_tmp_outs1_wrapper.allocate(pred_size * 2) ||
        !m_half_tmp_outs2_wrapper.allocate(pred_size * 2) ||
        !m_half_tmp_outs3_wrapper.allocate(pred_size * 2) ||
        !m_half_tmp_outs4_wrapper.allocate(pred_size * 2) ||
        !m_half_tmp_outs5_wrapper.allocate(pred_size * 2) ||
        // 添加临时包装器的内存分配(第二帧的输入)
        !m_half_temp_instance_feature_wrapper.allocate(temp_instance_feature_size) ||
        !m_half_temp_anchor_wrapper.allocate(temp_anchor_size) ||
    ...
    status_ = true;
    LOG(INFO) << "[INFO] SparseBEVFP16 module initialized successfully with GPU InstanceBank";
    return status_;
}
 ...
 * @brief 加载辅助数据
 */
void SparseBEVFP16::loadAuxiliaryData() 
{
    // 从TaskConfig获取空间形状数据
    ...
    size_t instance_feature_size = m_taskConfig.instance_bank_params().num_querys() * 
                                  m_taskConfig.model_cfg_params().embedfeat_dims();
    m_instance_feature.resize(instance_feature_size, __float2half(0.0f));
    
    // 从锚点文件加载锚点数据 - 不改变存储文件格式，进行float到half的转换
    m_anchor.clear();
    ...
    if (!anchor_path.empty()) {
        ...
        if (file.is_open()) {
            ...
            if (file_size == expected_file_size) {
                // 先读取到临时float数组
                std::vector<float> temp_anchor(expected_anchor_size);
                file.read(reinterpret_cast<char*>(temp_anchor.data()), file_size);
                
                // 转换为half精度
                m_anchor.resize(expected_anchor_size);
                for (size_t i = 0; i < expected_anchor_size; ++i) {
                    m_anchor[i] = __float2half(temp_anchor[i]);
                }
                ...
                m_anchor.resize(expected_anchor_size, __float2half(0.0f));
            }
            file.close();
        } else {
    ...
    // 从TaskConfig获取时间间隔
    m_time_interval.clear();
    m_time_interval.push_back(__float2half(m_taskConfig.instance_bank_params().default_time_interval()));
    
    // 从TaskConfig获取图像宽高（为6个相机分别保存）
    m_image_wh.resize(m_taskConfig.preprocessor_params().num_cams() * 2); // 6个相机 * 2个值（宽高）
    for (int i = 0; i < m_taskConfig.preprocessor_params().num_cams(); ++i) {
        m_image_wh[i * 2] = __float2half(static_cast<float>(m_taskConfig.preprocessor_params().model_input_img_w()));
        m_image_wh[i * 2 + 1] = __float2half(static_cast<float>(m_taskConfig.preprocessor_params().model_input_img_h()));
    }
    
    // 激光雷达到图像变换矩阵大小
    size_t lidar2img_size = m_taskConfig.preprocessor_params().num_cams() * 4 * 4;
    m_lidar2img.resize(lidar2img_size, __float2half(0.0f));
    
    // 初始化默认的单位矩阵作为标定参数
    for (int i = 0; i < m_taskConfig.preprocessor_params().num_cams(); ++i) {
        m_lidar2img[i * 16 + 0] = __float2half(1.0f);  // [0,0]
        m_lidar2img[i * 16 + 5] = __float2half(1.0f);  // [1,1]
        m_lidar2img[i * 16 + 10] = __float2half(1.0f); // [2,2]
        m_lidar2img[i * 16 + 15] = __float2half(1.0f); // [3,3]
    }
 ...
 * @param input 输入数据指针
 */
void SparseBEVFP16::setInput(void* input) {
    if (input == nullptr) {
...
void SparseBEVFP16::execute() 
{
    ...
    LOG(INFO) << "[INFO] Starting SparseBEVFP16 inference with GPU InstanceBank...";
    
    // 添加输入数据验证
    if (!m_half_input_wrapper.isValid()) {
        LOG(ERROR) << "[ERROR] Input wrapper is not valid!";
        return;
    }
    
    LOG(INFO) << "[INFO] Input wrapper size: " << m_half_input_wrapper.getSize();
    ...
    // 验证输出缓冲区
    if (!m_half_features_wrapper.isValid()) {
        LOG(ERROR) << "[ERROR] Features wrapper is not valid!";
        return;
    }
    
    LOG(INFO) << "[INFO] Features wrapper size: " << m_half_features_wrapper.getSize();
    ...
    try {
        ...
        result = extractFeatures(m_half_input_wrapper, stream, m_half_features_wrapper);
        ...
        cudaMemcpyAsync(m_gpu_instance_feature_wrapper.getCudaPtr(),
                        ...
                        std::min(m_gpu_instance_feature_wrapper.getSize(), gpu_instance_feature.getSize()) * sizeof(half),
                        cudaMemcpyDeviceToDevice, stream);

        // 拷贝锚点数据
        cudaMemcpyAsync(m_gpu_anchor_wrapper.getCudaPtr(),
                        gpu_anchor.getCudaPtr(),
                        std::min(m_gpu_anchor_wrapper.getSize(), gpu_anchor.getSize()) * sizeof(half),
                        cudaMemcpyDeviceToDevice, stream);
        ...
        if (!is_first_frame_ && has_previous_frame_) {
            // 拷贝缓存的GPU数据到时序数据包装器
            cudaMemcpyAsync(m_half_temp_instance_feature_wrapper.getCudaPtr(), 
                            gpu_cached_feature.getCudaPtr(),
                            std::min(m_half_temp_instance_feature_wrapper.getSize(), gpu_cached_feature.getSize()) * sizeof(half),
                            cudaMemcpyDeviceToDevice, stream);
            ...
            // 拷贝 anchor
            cudaMemcpyAsync(m_half_temp_anchor_wrapper.getCudaPtr(), 
                            gpu_cached_anchor.getCudaPtr(),
                            std::min(m_half_temp_anchor_wrapper.getSize(), gpu_cached_anchor.getSize()) * sizeof(half),
                            cudaMemcpyDeviceToDevice, stream);
        ...
        if (is_first_frame_ || !has_previous_frame_) {
            ...
            result = headFirstFrame(m_half_features_wrapper, stream,
                                  m_half_pred_instance_feature_wrapper,
                                  m_half_pred_anchor_wrapper,
                                  m_half_pred_class_score_wrapper,
                                  m_half_pred_quality_score_wrapper);
            ...
            if (result == Status::kSuccess) {
                ...
                Status status_cache = instance_bank_gpu_->cache(m_half_pred_instance_feature_wrapper,
                                             m_half_pred_anchor_wrapper,
                                             m_half_pred_class_score_wrapper,
            ...
            result = headSecondFrame(m_half_features_wrapper, stream,
                                    m_half_pred_instance_feature_wrapper,
                                    m_half_pred_anchor_wrapper,
                                    m_half_pred_class_score_wrapper,
                                    m_half_pred_quality_score_wrapper,
        ...
        // 从GPU获取数据并转换格式
        auto pred_instance_feature = m_half_pred_instance_feature_wrapper.cudaMemcpyD2HResWrap();
        auto pred_anchor = m_half_pred_anchor_wrapper.cudaMemcpyD2HResWrap();
        auto pred_class_score = m_half_pred_class_score_wrapper.cudaMemcpyD2HResWrap();
        auto pred_quality_score = m_half_pred_quality_score_wrapper.cudaMemcpyD2HResWrap();
 ...
 * @return 设置是否成功
 */
bool SparseBEVFP16::setInputDataFromWrapper(const sparsebev::SparseBEVInputData& input_data)
{
    // 设置图像特征
 ...
 * @brief 设置图像特征
 */
bool SparseBEVFP16::setImageFeature(const sparsebev::ImageFeature& image_data)
{
    if (!image_data.data_valid || std::holds_alternative<std::monostate>(image_data.feature)) {
    ...
    // 检查variant中的类型并安全访问
    if (auto half_ptr = std::get_if<std::shared_ptr<CudaWrapper<half>>>(&image_data.feature)) {
        // half类型的处理
        if (!*half_ptr) {
            LOG(ERROR) << "[ERROR] half feature pointer is null!";
            return false;
        }
        
        // 避免不必要的D2H->H2D，直接D2D拷贝到本模块输入包装器
        if (!m_half_input_wrapper.isValid() || m_half_input_wrapper.getSize() != (*half_ptr)->getSize()) {
            m_half_input_wrapper.allocate((*half_ptr)->getSize());
        }
        cudaMemcpyAsync(m_half_input_wrapper.getCudaPtr(),
                        (*half_ptr)->getCudaPtr(),
                        (*half_ptr)->getSize() * sizeof(half),
                        cudaMemcpyDeviceToDevice,
                        0);
    } else if (auto float_ptr = std::get_if<std::shared_ptr<CudaWrapper<float>>>(&image_data.feature)) {
        // float类型的处理 - 需要转换为half
        if (!*float_ptr) {
            LOG(ERROR) << "[ERROR] float feature pointer is null!";
            return false;
        }
        
        if (!m_half_input_wrapper.isValid() || m_half_input_wrapper.getSize() != (*float_ptr)->getSize()) {
            m_half_input_wrapper.allocate((*float_ptr)->getSize());
        }
        
        // 这里需要添加float到half的转换kernel调用
        LOG(WARNING) << "[WARNING] float to half conversion not implemented yet";
        return false;
    } else if (auto int8_ptr = std::get_if<std::shared_ptr<CudaWrapper<int8_t>>>(&image_data.feature)) {
        // int8_t类型的处理
        if (!*int8_ptr) {
            LOG(ERROR) << "[ERROR] int8_t feature pointer is null!";
            return false;
        }
        
        LOG(WARNING) << "[WARNING] int8_t to half conversion not implemented yet";
        return false;
    } else {
        LOG(ERROR) << "[ERROR] Unsupported feature type in variant!";
        return false;
    }
    
    cudaError_t err = cudaPeekAtLastError();
    if (err != cudaSuccess) {
        LOG(ERROR) << "[ERROR] D2D copy image feature failed: " << cudaGetErrorString(err);
        return false;
    }
 ...
 * @brief 设置时间间隔
 */
bool SparseBEVFP16::setTimeInterval(const sparsebev::TimeInterval& time_interval) 
{
    // 计算时间间隔
    if (has_previous_frame_) {
        // 时间戳单位为int64毫秒，时间间隔在0~2ms范围，按毫秒直接存储到half
        double time_diff_ms = std::abs(current_timestamp_ - previous_timestamp_);
        m_time_interval[0] = __float2half(static_cast<float>(time_diff_ms / 1000.0));
        
        LOG(INFO) << "[INFO] Calculated time interval (ms): " << __half2float(m_time_interval[0])
                  << " (from " << previous_timestamp_ << "ms to " << current_timestamp_ << "ms)";
    } else {
        m_time_interval[0] = __float2half(m_taskConfig.instance_bank_params().default_time_interval());
 ...
 * @brief 设置图像标定信息
 */
bool SparseBEVFP16::setImageCalibration(const sparsebev::ImageCalibration& image_calibration) 
{
    ...
    if (image_calibration.lidar2img && image_calibration.lidar2img->getSize() > 0) {
        ...
        // 将float数据转换为half并复制到m_lidar2img
        m_lidar2img.resize(lidar2img_data.size());
        for (size_t i = 0; i < lidar2img_data.size(); ++i) {
            m_lidar2img[i] = __float2half(lidar2img_data[i]);
        }
 ...
 * @brief 特征提取推理
 */
Status SparseBEVFP16::extractFeatures(const CudaWrapper<half>& input_imgs, 
 ...
 * @brief 第一帧头部推理
 */
Status SparseBEVFP16::headFirstFrame(const CudaWrapper<half>& features,
                                 const cudaStream_t& stream,
                                 ...
                                 CudaWrapper<half>& pred_quality_score) 
{
    ...
    std::vector<void*> output_buffers = {
        static_cast<void*>(m_half_tmp_outs0_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_tmp_outs1_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_tmp_outs2_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_tmp_outs3_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_tmp_outs4_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_tmp_outs5_wrapper.getCudaPtr()),
 ...
 * @brief 第二帧头部推理
 */
Status SparseBEVFP16::headSecondFrame(const CudaWrapper<half>& features,
                                  const cudaStream_t& stream,
                                  CudaWrapper<half>& pred_instance_feature,
                                  CudaWrapper<half>& pred_anchor,
                                  CudaWrapper<half>& pred_class_score,
                                  ...
                                  CudaWrapper<int32_t>& pred_track_id)
{
    ...
    std::vector<void*> input_buffers = {
        ...
        static_cast<void*>(m_gpu_anchor_wrapper.getCudaPtr()),
        static_cast<void*>(m_gpu_time_interval_wrapper.getCudaPtr()),
        static_cast<void*>(m_half_temp_instance_feature_wrapper.getCudaPtr()),  // 修正变量名
        static_cast<void*>(m_half_temp_anchor_wrapper.getCudaPtr()),           // 修正变量名
    ...
    //     LOG(INFO) << "[INFO] Time interval saved successfully";
    // }
    // if (savePartialFast(m_half_temp_instance_feature_wrapper, 153600, "/share/Code/Sparse4d/C++/Output/val_bin_gpu/sample_1_input_temp_instance_feature_1*600*256_float32.bin")) {
    //     LOG(INFO) << "[INFO] temp_instance_feature saved successfully";
    // }
    // if (savePartialFast(m_half_temp_anchor_wrapper, 6600, "/share/Code/Sparse4d/C++/Output/val_bin_gpu/sample_1_input_temp_anchor_1*600*11_float32.bin")) {
 ...
 * @brief 转换输出格式
 */
void SparseBEVFP16::convertToOutputFormat(const std::vector<half>& pred_instance_feature,
...
{
    // 将half数据转换为float
    std::vector<float> pred_instance_feature_f(pred_instance_feature.size());
    for (size_t i = 0; i < pred_instance_feature.size(); ++i) {
        pred_instance_feature_f[i] = __half2float(pred_instance_feature[i]);
    }
    std::vector<float> pred_anchor_f(pred_anchor.size());
    for (size_t i = 0; i < pred_anchor.size(); ++i) {
        pred_anchor_f[i] = __half2float(pred_anchor[i]);
    }
    std::vector<float> pred_class_score_f(pred_class_score.size());
    for (size_t i = 0; i < pred_class_score.size(); ++i) {
        pred_class_score_f[i] = __half2float(pred_class_score[i]);
    }
    std::vector<float> pred_quality_score_f(pred_quality_score.size());
    for (size_t i = 0; i < pred_quality_score.size(); ++i) {
        pred_quality_score_f[i] = __half2float(pred_quality_score[i]);
    }

    // 创建CudaWrapper并复制数据（float）
    raw_result.pred_instance_feature = std::make_shared<CudaWrapper<float>>(pred_instance_feature_f);
    raw_result.pred_anchor = std::make_shared<CudaWrapper<float>>(pred_anchor_f);
    raw_result.pred_class_score = std::make_shared<CudaWrapper<float>>(pred_class_score_f);
    raw_result.pred_quality_score = std::make_shared<CudaWrapper<float>>(pred_quality_score_f);
 ...
 * @brief 获取原始推理结果
 */
RawInferenceResult SparseBEVFP16::getRawInferenceResult() const 
{
    return m_raw_result;
}
 ...
 * @brief 设置当前样本索引
 */
void SparseBEVFP16::setCurrentSampleIndex(int sample_index) {
 ...
 */
template<typename T>
bool SparseBEVFP16::saveDataToFile(const CudaWrapper<T>& data, const std::string& file_path) {
    if (!data.isValid()) {
 ...
 */
template<typename T>
bool SparseBEVFP16::savePartialFast(const CudaWrapper<T>& gpu, size_t effective_elems, const std::string& path) {
