...
class InstanceBankGPU {
  public:
    ...
    std::tuple<const CudaWrapper<half>&,
                const CudaWrapper<half>&,
                const CudaWrapper<half>&,
                const CudaWrapper<half>&,
                const half,
                const std::int32_t,
                const CudaWrapper<std::int32_t>&>
    get(const int64_t& timestamp, const Eigen::Matrix<double, 4, 4>& global_to_lidar_mat, const bool& is_first_frame, cudaStream_t stream = 0);
    ...
    /// @param is_first_frame 是否为第一帧
    Status cache(const CudaWrapper<half>& instance_feature,
 ...
 private:
    ...
    // std::vector<half> kmeans_anchors_; 
    uint32_t topk_anchors_;                     // 前k个锚点数量 600
    half max_time_interval_;                   // 最大时间间隔 2.0
    half default_time_interval_;               // 默认时间间隔 0.5
    half confidence_decay_;                    // 置信度衰减因子
    uint32_t embedfeat_dims_;                   // 实例特征维度 256

    // 实例库状态
    std::int32_t mask_;
    int64_t history_time_;                        // 上一帧时间戳
    half time_interval_;                        // 时间间隔
