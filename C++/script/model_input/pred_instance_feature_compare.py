...
def compare_features(data1, data2, tolerance=1e-6):
    ...
    return stats, column_max_errors, instance_max_errors

def main():
    # 文件路径
    file1_path = "/share/Code/SparseEnd2End/C++/Output/val_bin/sample_0_pred_instance_feature_1*900*256_float32.bin"
    file2_path = "/share/Code/SparseEnd2End/script/tutorial/asset/sample_0_pred_instance_feature_1*900*256_float32.bin"
    
    # 数据形状
    shape = (1, 900, 256)  # (batch_size, num_instances, feature_dim)
    
    print("=== Pred Instance Feature 比较脚本 ===")
    ...
    # 比较数据
    stats, column_max_errors, instance_max_errors = compare_features(data1, data2)
    ...
    # 保存详细结果到文件
    output_file = "pred_instance_feature_comparison_results.txt"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("=== Pred Instance Feature 比较结果 ===\n")
        ...
        f.write(f"\n=== 前20个最大误差的实例 ===\n")
        for i, inst_stats in enumerate(instance_max_errors[:20]):
