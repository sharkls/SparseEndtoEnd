...
class Sparse4D(BaseDetector):
    ...
    def extract_feat(self, img, return_depth=False, metas=None):
        ...
        if img.dim() == 5:  # multi-view
            ...
            num_cams = 1
        if self.use_grid_mask:      # 对图像增加掩码
            img = self.grid_mask(img)
        if "metas" in signature(self.img_backbone.forward).parameters:
            feature_maps = self.img_backbone(img, num_cams, metas=metas)
        else:
            feature_maps = self.img_backbone(img)    # 6, 3, 256, 704 => [6, 2048, 8, 22]# # [6, 256, 64, 176], [6, 512, 32, 88], [6, 1024, 16, 44], [6, 2048, 8, 22]
        if self.img_neck is not None:
            feature_maps = list(self.img_neck(feature_maps)) # =>[6, 256, 64, 176], [6, 256, 32 ,88],[6, 256, 16, 44], [6, 256, 8, 22]
        for i, feat in enumerate(feature_maps):
            feature_maps[i] = torch.reshape(feat, (bs, num_cams) + feat.shape[1:])  # 6, 256, 64, 176 => 1, 6, 256, 64, 176
        if return_depth and self.depth_branch is not None:
            depths = self.depth_branch(feature_maps, metas.get("focal")) # [6,1,64,176], [6,1,32,88],[6,1,16,44]基于相机焦距计算深度
        else:
            depths = None
        if self.use_deformable_func:
            feature_maps = feature_maps_format(feature_maps)  # => col_feats[1,89760,256], spatial_shape[1,6,4,2], scale_start_index[6,4]
        if return_depth:
    ...
    def forward_train(self, img, **data):
        feature_maps, depths = self.extract_feat(img, True, data) # {col_feats[1,89760,256], spatial_shape[1,6,4,2], scale_start_index[6,4]}, {[6,1,64,176], [6,1,32,88],[6,1,16,44]}
