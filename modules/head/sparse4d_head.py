...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.anchor_encoder = build_module(anchor_encoder)  # pos size yaw vel + nn.linear
    ...
    def forward(
        ...
        if self.training and hasattr(self.sampler, "get_dn_anchors"):
            if "track_id" in metas["img_metas"][0]:
                ...
                gt_track_id = None
            dn_metas = self.sampler.get_dn_anchors( # 根据随机噪音生成正样本和负样本
        ...
        if dn_metas is not None:
            (
                dn_anchor,          # [1, 170, 10] 一半是正样本噪声，一半是负样本噪声，其中170=num_gt*2*num_dn_groups
                dn_reg_target,      # [1, 170, 10] 一半是正样本噪声，一半是负样本噪声
                dn_cls_target,      # [1, 170] 一半是正样本噪声，一半是负样本噪声(-3)
                dn_attn_mask,       # [170, 170] 多个self.num_dn_groups对角为False
                valid_mask,         # [1, 170] 正样本噪声和负样本噪声都为True
                dn_id_target,       # [1, 170]  正样本id 和 负样本id(-1)
            ) = dn_metas
            ...
            if dn_anchor.shape[-1] != anchor.shape[-1]:
                ...
                dn_anchor = torch.cat(  # [1, 170, 10] -> [1, 170, 11]
                    [
                        dn_anchor,
                        dn_anchor.new_zeros(
            ...
            anchor = torch.cat([anchor, dn_anchor], dim=1)  # (bs, 320+900, 11) # 将anchor[1, 900, 11]和根据标签和噪声生成正负样本[1, num_gt*2*num_dn_groups, 11]的dn_anchor拼接
            instance_feature = torch.cat(                   # 将instance_feature[1, 900, 256]和根据标签和噪声生成正负样本[1, num_gt*2*num_dn_groups, 256]的instance_feature拼接
                [
                    instance_feature,
            ...
            num_free_instance = num_instance - num_dn_anchor  # 320+900-320=900 anchor原始数量
            attn_mask = anchor.new_ones((num_instance, num_instance), dtype=torch.bool)
            attn_mask[:num_free_instance, :num_free_instance] = False   # 注意力掩码图中 前900个实例之间为False
            attn_mask[num_free_instance:, num_free_instance:] = (       # 后num_gt*2*num_dn_groups个实例(噪声)之间是采用dn_attn_mask
                dn_attn_mask  # (1120, 1120)
            )

        anchor_embed = self.anchor_encoder(anchor)  # (bs, 320+900, 256)   # 对3D anchor进行编码 [1, 1040(900+num_gt*2*num_dn_groups), 11] --> [1, 1040,256]
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                quality.append(qt)
                if len(prediction) == self.num_single_frame_decoder:    # 如果当前是第一帧一个decoder，则更新instance_bank
