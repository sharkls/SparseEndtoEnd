...
class SparseBox3DEncoder(BaseModule):
    ...
    def forward(self, box_3d: torch.Tensor):
        pos_feat = self.pos_fc(box_3d[..., X:W])            # [1, 1040(900+140), 3] --> [1, 1040, 128] 
        size_feat = self.size_fc(box_3d[..., W:SIN_YAW])    # [1, 1040(900+140), 3] --> [1, 1040, 32] 
        yaw_feat = self.yaw_fc(box_3d[..., SIN_YAW:VX])     # [1, 1040(900+140), 3] --> [1, 1040, 32] 
        if self.mode == "add":
        ...
        if self.vel_dims > 0:
            vel_feat = self.vel_fc(box_3d[..., VX : VX + self.vel_dims])  # # [1, 1040(900+140), 3] --> [1, 1040, 64] 
            if self.mode == "add":
...
class SparseBox3DRefinementModule(BaseModule):
    ...
    def forward(
        ...
        if return_cls and self.with_quality_estimation:
            ...
            quality = None
        return output, cls, quality # [1, 900+x, 11], [1, 900+x, 10], [1, 900+x, 2]
...
class SparseBox3DKeyPointsGenerator(BaseModule):
    ...
    def forward(
        ...
        key_points = self.fix_scale * size  # (bs, num_query, 7, 3) # 获取anchor的7个固定点（6面+1中心）
        # self.num_learnable_pts default=0 在DFA模块设置为6
        if self.num_learnable_pts > 0 and instance_feature is not None:
            learnable_scale = (
                self.learnable_fc(instance_feature) # [1, 900+x, 256] --> [1, 900+x, 6*3]
                .reshape(bs, num_anchor, self.num_learnable_pts, 3) # [1, 900+x, 6*3] --> [1, 900+x, 6, 3]
                .sigmoid() 
                - 0.5 
            )  # (bs, num_query, 6, 3)
        ...
        rotation_mat = anchor.new_zeros([bs, num_anchor, 3, 3]) # [1, 900+x, 3, 3]
        ...
        key_points = torch.matmul(rotation_mat[:, :, None], key_points[..., None])[ # 对关键点进行旋转 -> [1, 900+x, 6+7, 3]
            ..., 0
        ]  # deploy friendly

        # [bs,num_query,6+7,3] + [bs, num_query, 1, 3]
        # key_points = key_points + anchor[..., None, [X, Y, Z]]
        key_points = key_points + anchor[..., None, :3]  # deploy friendly # 关键点加上anchor的中心点
        return key_points
    ...
    def anchor_projection(
        ...
        T_src2dst_list : list default = 1?  [(1,4,4)]   上一帧数据的global坐标系到当前帧global坐标系的旋转平移矩阵
