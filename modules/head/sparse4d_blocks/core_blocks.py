...
class DeformableAttentionAggr(BaseModule):
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        Return:
            out: (bs, 900+x, 512)
        """
        bs, num_anchor = instance_feature.shape[:2]     # 1， 900+x
        # [bs, num_anchors, 7+6, 3]
        key_points = self.kps_generator(anchor, instance_feature) # 获取anchor的7个固定点（6面+1中心）和6个可学习点 [1, 900+x, 13, 3]
        # [bs, num_anchors, 6, 4, 13, 8]
        weights = self._get_weights(instance_feature, anchor_embed, metas) # 根据实例特征和anchor特征和相机外参获取权重 [1, 900+x, 6, 4, 13, 8]

        if self.use_deformable_func:
            points_2d = (   # 投影至2D图像平面 [1,900+x,13,6,2]
                self.project_points(
                    key_points,
            ...
            weights = (  # 将权重重塑为[1,900+x,13,6,4,8]
        ...
        output = self.proj_drop(self.output_proj(features)) # nn.linear(bs, 900+x, 256) => (bs, 900+x, 256)
        if self.residual_mode == "add":
            output = output + instance_feature
        elif self.residual_mode == "cat":
            output = torch.cat([output, instance_feature], dim=-1)  # (bs, 900+x, 512)
        return output
    ...
    def _get_weights(self, instance_feature, anchor_embed, metas=None):
        ...
        """
        bs, num_anchor = instance_feature.shape[:2] # 1, 900+x
        feature = instance_feature + anchor_embed # [1, 900+x, 256] + [1, 900+x, 256] => [1, 900+x, 256]融合实例特征和anchor特征
        if self.camera_encoder is not None:
            ...
            feature = feature[:, :, None] + camera_embed[:, None] # [1, 900+x, 1, 256] + [1, 1, 6, 256] => [1, 900+x, 6, 256]融合相机外参特征
        ...
        if self.training and self.attn_drop > 0:
            ...
            weights = ((mask > self.attn_drop) * weights) / (1 - self.attn_drop)        # ？todo
        return weights
    ...
    def project_points(key_points, lidar2img, image_wh=None):
        ...
        # ).squeeze(-1)
        points_2d = torch.matmul(   # 将3D点投影到2D图像平面[1,6,1,1,4,4] @ [1,1,900+x,13,4,1] => [1,6,900+x,13,4]
            lidar2img[:, :, None, None], pts_extend[:, None, ..., None]
        )[..., 0]
        points_2d = points_2d[..., :2] / torch.clamp(points_2d[..., 2:3], min=1e-5) # 归一化[1,6,900+x,13,2]
        if image_wh is not None:
            points_2d = points_2d / image_wh[:, :, None, None] # 归一化[1,6,900+x,13,2]
        return points_2d    # [1,6,900+x,13,2]
...
class GridMask(nn.Module): # 图像掩码用于数据增强，防止模型过分依赖特定区域
    def __init__(
        self, use_h, use_w, rotate=1, offset=False, ratio=0.5, mode=0, prob=1.0
    ):
    ...
    def forward(self, x):
        if np.random.rand() > self.prob or not self.training:
            return x
        n, c, h, w = x.size()
        x = x.view(-1, h, w)    # 6, 3, 256, 704 => 18, 256, 704
        hh = int(1.5 * h)
        ww = int(1.5 * w)
...
#     def __init__(
#         self, use_h, use_w, rotate=1, offset=False, ratio=0.5, mode=0, prob=1.0, save_dir=None, save_count=0
#     ):
#         super(GridMask, self).__init__()
#         self.use_h = use_h
#         self.use_w = use_w
...
#         x_masked = x.view(n, c, h, w)

#         # 保存图像
#         if self.save_dir is not None and self.save_count > 0:
#             os.makedirs(self.save_dir, exist_ok=True)
#             for i in range(min(self.save_count, n)):
#                 original_image = vutils.make_grid(x[i].unsqueeze(0), normalize=True, scale_each=True)
#                 masked_image = vutils.make_grid(x_masked[i].unsqueeze(0), normalize=True, scale_each=True)
#                 vutils.save_image(original_image, os.path.join(self.save_dir, f"original_{self.save_idx}.png"))
#                 vutils.save_image(masked_image, os.path.join(self.save_dir, f"masked_{self.save_idx}.png"))
#                 self.save_idx += 1
