...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def _box_cost(self, box_pred, box_target, instance_reg_weights):
        ...
        for i in range(bs):
            ...
            if len(box_target[i]) > 0:
                cost.append(
                    torch.sum(
                        torch.abs(box_pred[i, :, None] - box_target[i][None])   # [34, 1, 10] - [1, 17,10] = [34, 17, 10]
                        * instance_reg_weights[i][None]                         # [1, 17, 10]
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_track_id=None):
        ...
        box_target = self.encode_reg_target(box_target, cls_target.device)   #  将角度信息分解为sin cos
        ...
        bs, num_gt, state_dims = box_target.shape  # 1，17，10
        ...
        dn_box_target = torch.zeros_like(           # [5, 34, 10]
            dn_anchor
        ...
        return (
            dn_anchor,          # [1, 170, 10] 一半是正样本噪声，一半是负样本噪声
            dn_box_target,      # [1, 170, 10] 一半是正样本噪声，一半是负样本噪声
            dn_cls_target,      # [1, 170] 一半是正样本噪声，一半是负样本噪声(-3)
            attn_mask,          # [170, 170] 多个self.num_dn_groups对角为False
